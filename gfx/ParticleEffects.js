...
export function explode(parent, cellSize, viewSize, time, cont) {
    let system = new Partykals.ParticlesSystem({
        container: parent,
        particles: {
            globalSize: 20,
            ...
            //alpha: 1,
            startColor: new THREE.Color(0xffffff),
            endColor: new THREE.Color(0x999999),
            startAlpha: 0.9,
            endAlpha: 0,
            //startAlphaChangeAt : 0,
            blending: 'blend',
            velocity: new Partykals.Randomizers.SphereRandomizer(30, 10),
            velocityBonus: new THREE.Vector3(0, 0, 5),
            gravity: 0,
            ttl: time * 1,
            ttlExtra : time * (cont ? 1.5 : 0.2),
            offset: new Partykals.Randomizers.BoxRandomizer(new THREE.Vector3(-cellSize/5, -cellSize/5, 0), new THREE.Vector3(cellSize/5, cellSize/5, 20)),
            worldPosition: true
        },
        system: {
            particlesCount: viewSize * (cont ? 3 : 1),
            emitters: cont ? new Partykals.Emitter({
                    onInterval: viewSize * 2 / 100,
                    interval: time / 100,
                }) : new Partykals.Emitter({
                    onSpawnBurst: viewSize
                }),
            speed: 1,
            ttl: time * 1.5,
            perspective: true,
